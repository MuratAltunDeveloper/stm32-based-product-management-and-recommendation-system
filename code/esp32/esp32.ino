...
// WiFi bilgileri
const char* ssid ; //Wi-Fi ağınızın SSID'sini (isim) yazmanız gerekir.
const char* password ; //internet sifrenizi giriniz


// API bilgileri
const char* Gemini_Token ; //Gemini token buraya girilir
 ...
 // WiFi bağlantı durumunu kontrol et ve gerekirse yeniden bağlan
  ...
  // !!!!!!!!!!!!!!!!STM32'den veri al
    if (SerialPort.available()) {
        ...
        Serial.println(receivedData); // PC'ye alınan veriyi yazdır
        delay(2000);
 Serial.println("\n kontrol2 \n");
...
esp32response+="finish";
   // !!!!!!!!!!!!!!!!!!!!!!!!!!!STM32'ye veri gönder
    Serial.println(esp32response);
    SerialPort.println(esp32response);
